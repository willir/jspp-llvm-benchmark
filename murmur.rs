...
pub extern "C" fn murmur_rust(key: *const Arr, seed: u32) -> u32 {
    ...
    if key.len() > 3 {
        ...
        for i in (0..len).step_by(4) {
            ...
            k = k.overflowing_mul(0xcc9e2d51).0;
            k = k.rotate_left(15);
            k = k.overflowing_mul(0x1b873593).0;
            h ^= k;
            h = h.rotate_left(13);
            h = h.overflowing_mul(5).0.overflowing_add(0xe6546b64).0;
        }
    }
    if (key.len() & 3) != 0 {
        let mut i = key.len() & 3;
    ...
    h ^= key.len() as u32;
    h ^= h >> 16;
    h = h.overflowing_mul(0x85ebca6b).0;
    h ^= h >> 13;
    h = h.overflowing_mul(0xc2b2ae35).0;
    h ^= h >> 16;
...
fn buf_to_k(chunk: [u8; 4]) -> u32 {
    u32::from_le_bytes(chunk)
        .overflowing_mul(0xcc9e2d51)
        .0
        .rotate_left(15)
        .overflowing_mul(0x1b873593)
        .0
}
